...
class VideoModel extends Model {
    ...
    public function listVideo($firstRow = 0, $listRows = 20 , $filter = array()) {
		...
		{
			$where .= " AND v.title LIKE '%" . mysql_like_quote($filter['keyword']) . "%'";
		}
		if ($filter['cat_id'])
		{
		...
		/* 获取文章数据 */
		$sql = 'SELECT v.video_id, v.cat_id, v.title, v.videoType, v.sort_order,v.add_time ,v.is_open,v.is_hot,v.thumb_img, vc.cat_name '.
    ...
    public function listVideoCount($filter = array()) {
        ...
        $sql = 'SELECT COUNT(video_id) AS count FROM ' . C('DB_PREFIX') . 'video AS v '.
               'WHERE 1 ' .$where;
