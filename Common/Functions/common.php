 ...
 */
function getTree($list,$id="cat_id") {
  $tree = array();
  if(is_array($list)) {
    $refer = array();
...
/**
	...
	* @param   int     		$cat_id     当前选中分类的ID
	* @param   int     		$level     	级别
 */
function optionsDate($list,$selected,$disabled=0,$level=0,$id="cat_id",$name="cat_name"){
	$tree = '';
	foreach($list as $key=>$value){
		$tree .= '<option value="'.$value[$id].'"';
		...
		$tree .= '>'.str_repeat('&nbsp;',$level*4).htmlspecialchars(addslashes($value[$name])).'</option>>';
		if(!empty($value['child'])){
			$tree .= optionsDate($value['child'],$selected,$disabled,$level+1,$id,$name);
		}
	}
	return $tree;
}
...
function db_create_not_in($item_list, $field_name = '')
{
	...
	**获得指定分类下所有底层分类的ID
	 */
	function get_child_id($parent_id,$mode,$id){
		$car_arr = array();
		$cat_arr[] = $parent_id;
		foreach(get_child($parent_id,$mode,$id) as $v){
			$cat_arr[] = $v[$id];
		}
		return db_create_in(array_unique($cat_arr),$id);
	}
	function get_child($parent_id,$mode,$id){
		$arr = array();
		$arr = M($mode)->where("parent_id=$parent_id")->select();
		foreach($arr as $v){
			$arr2 = get_child($v[$id],$mode,$id);
			if(!empty($arr2)){
...
function water($img){
	...
	$Image->water($img,'Public/water.png','',60);
}
