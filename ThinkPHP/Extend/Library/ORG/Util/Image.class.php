...
class Image {
     ...
     * @return void
     */
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true,$fixed = true,$thumbType=1) {
        // 获取原图信息
        ...
        if ($info !== false) {
			...
			if ($maxHeight > $srcHeight && $maxWidth > $srcWidth) {
				if(($maxWidth/$maxHeight)>($srcWidth/$srcHeight)){
					...
					$oy = ($maxHeight-$height)/2;
					$ox = 0;
				}
            }else{
				$ox = ($maxWidth - $srcWidth * $scale) / 2;
				$oy = ($maxHeight -$srcHeight * $scale) / 2;
			}
            ...
            if (function_exists("ImageCopyResampled"))
                imagecopyresampled($thumbImg, $srcImg, $ox, $oy, 0, 0, $width, $height, $srcWidth, $srcHeight);
            else
                imagecopyresized($thumbImg, $srcImg, $ox, $oy, 0, 0, $width, $height, $srcWidth, $srcHeight);
