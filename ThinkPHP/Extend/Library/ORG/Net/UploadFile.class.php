...
class UploadFile {//类定义开始
    ...
    private $config =   array(
        ...
        'thumbType'         =>  1,// 缩略图裁剪类型，1-缩略图等比例缩放填充空白，2-剧中剪切
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    Image::thumb($filename,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true,$this->is_fixed,$this->thumbType);                    
                }
    ...
    private function getSaveName($filename) {
        ...
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
			...
			if(function_exists($rule)) {
				if('time' == $rule){
					$saveName = $rule().$filename['key'].".".$filename['extension'];
				}
				else{
